...
function toBindLink(){
    var p = common.removeParamObject(["from","bind", "code", "share_id", "isappinstalled", "state", "m", "c", "a"])+common.addParamHsah();
...
function checkBindAndBind(){
    ...
    if(b&&o) {
        ...
        function(x) {
            ...
            location.href = location.origin +common.removeParamFromUrl(["bind","code"])+common.addParamHsah();
        })
    }
}
...
function toRegisterAndBack(){
    ...
    location.href=MasterConfig.C('basePageUrl')+"person/index.html?start=123#/register?comeFrom="+encodeURIComponent(n);
}
